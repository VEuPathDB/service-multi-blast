...
internal class GIListTest {

  @Nested
  @DisplayName("ParseGIList()")
  inner class Parse {
    ...
    inner class NonString {
      ...
      fun t1() {
        assertThrows<IllegalArgumentException> {
          ParseGIList(Json.new { put(FlagGIList, 40) })
        }
      }
    }
    ...
    inner class Absent {
      ...
      fun t1() {
        assertTrue(ParseGIList(Json.new {}).isDefault)
      }
    }
    ...
    inner class Present {
      ...
      fun t1() {
        val inp = Json.newObject { put(FlagGIList, "bunny") }
        assertEquals("bunny", ParseGIList(inp).file)
      }
    }
  }
  ...
  inner class Type {
    ...
    inner class Arg {
      ...
      fun t1() {
        assertFalse(GIList("bye.db").isDefault)
      }
      ...
      fun t2() {
        val inp = Json.newObject()
        GIList("goop.txt").appendJson(inp)
        assertEquals("""{"$FlagGIList":"goop.txt"}""", inp.toString())
      }
      ...
      fun t3() {
        ...
        GIList("nope.nope.nope").appendCliSegment(inp)
        assertEquals(" $FlagGIList 'nope.nope.nope'", inp.toString())
      }
      ...
      fun t4() {
        ...
        GIList("yup.yup.yup").appendCliParts(inp)
        assertEquals(2, inp.size)
        assertEquals(FlagGIList, inp[0])
