...
enum class RPSTBlastNCompBasedStats {
  None,
  CompBasedStats,
  ;
  ...
  companion object {
    ...
    @JvmStatic
    @JsonCreator
    fun fromJson(js: JsonNode): RPSTBlastNCompBasedStats {
      js.dEnumRequireTextual(RPSTBlastNCompBasedStats::class)
      return when (val v = js.textValue()) {
        "none"                                      -> None
        "comp-based-stats"                          -> CompBasedStats
        else                                        -> throw UnrecognizedEnumException(RPSTBlastNCompBasedStats::class, v)
      }
    }
  }
