...
enum class TBlastXMatrix {
  ...
  companion object {
    ...
    @JvmStatic
    @JsonCreator
    fun fromJson(js: JsonNode): TBlastXMatrix {
      js.dEnumRequireTextual(TBlastXMatrix::class)

      for (v in values())
        if (js.textValue() == v.name)
          return v

      throw UnrecognizedEnumException(TBlastXMatrix::class, js.textValue())
    }
  }
