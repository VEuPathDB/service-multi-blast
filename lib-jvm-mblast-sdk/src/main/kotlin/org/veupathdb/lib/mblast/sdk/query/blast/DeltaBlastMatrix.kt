...
enum class DeltaBlastMatrix {
  ...
  companion object {
    ...
    @JvmStatic
    @JsonCreator
    fun fromJson(js: JsonNode): DeltaBlastMatrix {
      js.dEnumRequireTextual(DeltaBlastMatrix::class)

      for (v in values())
        if (js.textValue() == v.name)
          return v

      throw UnrecognizedEnumException(DeltaBlastMatrix::class, js.textValue())
    }
  }
}