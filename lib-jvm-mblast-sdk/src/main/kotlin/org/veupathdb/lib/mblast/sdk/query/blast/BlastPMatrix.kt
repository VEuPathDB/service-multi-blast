...
enum class BlastPMatrix {
  ...
  companion object {
    ...
    @JvmStatic
    @JsonCreator
    fun fromJson(js: JsonNode): BlastPMatrix {
      js.dEnumRequireTextual(BlastPMatrix::class)

      for (v in values())
        if (js.textValue() == v.name)
          return v

      throw UnrecognizedEnumException(BlastPMatrix::class, js.textValue())
    }
  }
}