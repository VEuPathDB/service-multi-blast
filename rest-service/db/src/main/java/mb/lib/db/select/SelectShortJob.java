...
public class SelectShortJob
{
  private final byte[] hash;

  public SelectShortJob(byte[] hash) {
    this.hash = hash;
  }

  public Optional<ShortJobRow> execute(CheckedSupplier<Connection> provider) throws Exception {
  ...
  Optional<ShortJobRow> parse(ResultSet rs) throws Exception {
    if (!rs.next())
      return Optional.empty();

    return Optional.of(JobRowFactory.newShortJobRow(
      this.hash,
